...
def run_prediction_experiment(env, episodes=1000, runs=30, gamma=0.99, alpha=0.1):
    ...
    # 위쪽 그래프: Mean of V(s)
    # Monte Carlo는 시간이 지나면서 더 정답에 가까운 값에 수렴
    # TD(0)는 좀 더 편향되어 있지만, 빠르게 수렴할 수 있음
    # 아래쪽 그래프: Variance of V(s)
    # Monte Carlo는 분산이 큼 (에피소드 전체의 결과에 의존)
    # TD(0)는 분산이 작음 (부트스트래핑 덕분에 안정적 업데이트)

    # Monte Carlo는 정확하지만 안정성이 떨어지고,
    # TD(0)는 빠르고 안정적이지만 약간의 편향이 생긴다.
